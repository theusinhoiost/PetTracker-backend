...
export class UserService {
  ...
  async create(dto: CreateUserDto) {
    ...
    const existsPhone = await this.userRepository.exists({ where: { phoneNumber: dto.phoneNumber } })
    if (existsEmail || existsPhone) { throw new ConflictException("Email ou telefone inválidos") }
  ...
  async updatePassword(id: string, dto: UpdatePasswordDto) {
    ...
    if (!isCurrentPasswordValid) throw new UnauthorizedException("Senha inválida")
